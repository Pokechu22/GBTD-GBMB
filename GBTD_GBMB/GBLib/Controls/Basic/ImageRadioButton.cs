...
namespace GB.Shared.Controls
{
	...
	public class ImageRadioButton : Control
	{
		...
		[Category("Data"), Description("Whether or not this is checked.")]
		public bool Checked {
			get { return @checked; }
			set { @checked = value; OnCheckedChanged(new EventArgs()); this.Invalidate(true); }
		}
		private bool autoCheck = true;
		[Category("Behavior"), Description("Whether or not to automatically uncheck others radiobuttons.")]
		[DefaultValue(true)]
		public bool AutoCheck {
			get { return autoCheck; }
			set { autoCheck = value; this.Invalidate(true); }
		}
		...
		/// <summary>
		/// Sets all other RadioButtons to unchecked.
		/// </summary>
		protected void UncheckAllOthers() {
			if (autoCheck) {
				if (this.Parent != null) {
					foreach (Control c in this.Parent.Controls) {
						...
						if (c is ImageRadioButton) {
							(c as ImageRadioButton).Checked = false;
						}
					}
				}
			}
